...
//     [image drawInRect:CGRectMake(0, 416, 320, 44)];
 }
...
- (id)initWithImage:(UIImage*)newImage andMaxSize:(CGSize)maxSize {
    ...
    if (self) {
        ...
        //Set center for frame : 
        CGRect frame = [self calcFrameWithImage:newImage andMaxSize:maxSize];
        
        self.frame = CGRectMake((maxSize.width - frame.size.width)/2, 0.0, frame.size.width, frame.size.height);
...
- (CGRect)calcFrameWithImage:(UIImage*)image andMaxSize:(CGSize)maxSize {
    ...
    if (((maxSize.height - increase) / image.size.height) < ((maxSize.width - increase) / image.size.width)) {
        imageScale = (maxSize.height - increase) / image.size.height;
    }
    else {
...
- (void)setup {
    ...
    cropView = [CropperView initialCropViewForImageView:imageView];
...
//    uvViewImage.transform = CGAffineTransformConcat(CGAffineTransformMakeRotation(currDegree), newTransform); 
